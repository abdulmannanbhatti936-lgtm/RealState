<?php
$page_title = "Edit Property";
require_once 'includes/header.php';

// Check if ID is provided
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    echo "<script>window.location.href='dashboard.php';</script>";
    exit();
}

$id = $_GET['id'];

// Fetch current property data
$stmt = $conn->prepare("SELECT * FROM properties WHERE id = ?");
$stmt->bind_param("i", $id);
$stmt->execute();
$result = $stmt->get_result();

if ($result->num_rows === 0) {
    echo "<script>window.location.href='dashboard.php';</script>";
    exit();
}

$property = $result->fetch_assoc();

// Handle Form Submission
if (isset($_POST['submit'])) {
    $title = mysqli_real_escape_string($conn, $_POST['title']);
    $location = mysqli_real_escape_string($conn, $_POST['location']);
    $price = $_POST['price'];
    $description = mysqli_real_escape_string($conn, $_POST['description']);
    $status = $_POST['status'];

    $image = $property['image']; // Default to old image
    $error = null;

    // Check if new image is uploaded
    if (!empty($_FILES['image']['name'])) {
        $image_file = $_FILES['image'];
        $allowed_types = ['image/jpeg', 'image/png', 'image/jpg'];

        if (!in_array($image_file['type'], $allowed_types)) {
            $error = "Invalid file type. Only JPG, JPEG, and PNG are allowed.";
        } elseif ($image_file['size'] > 5000000) {
            $error = "File is too large. Maximum size is 5MB.";
        } else {
            $image = time() . '_' . basename($image_file['name']);
            $target = "../assets/images/" . $image;
            if (!move_uploaded_file($image_file['tmp_name'], $target)) {
                $error = "Failed to upload image.";
            }
        }
    }

    if (!$error) {
        $stmt = $conn->prepare("UPDATE properties SET title = ?, location = ?, price = ?, image = ?, description = ?, status = ? WHERE id = ?");
        $stmt->bind_param("ssdsssi", $title, $location, $price, $image, $description, $status, $id);

        if ($stmt->execute()) {
            echo "<script>window.location.href='dashboard.php?msg=Listing updated successfully!';</script>";
            exit();
        } else {
            $error = "Database error: " . $conn->error;
        }
    }
}
?>

<main class="container" style="padding: 40px 0;">
    <div class="admin-card" style="max-width: 900px; margin: 0 auto;">
        <div
            style="margin-bottom: 35px; border-bottom: 2px solid #f1f5f9; padding-bottom: 20px; display: flex; align-items: center; gap: 15px;">
            <div
                style="width: 50px; height: 50px; background: rgba(52, 152, 219, 0.1); color: #3498db; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 1.5rem;">
                <i class="fa-solid fa-pen-to-square"></i>
            </div>
            <div>
                <h2 style="font-weight: 700; color: var(--primary-color);">Edit Property Listing</h2>
                <p style="color: #64748b; font-size: 0.9rem;">Modify the details for property ID: #<?php echo $id; ?>
                </p>
            </div>
        </div>

        <?php if (isset($error)): ?>
            <div
                style="background: #fee2e2; color: #b91c1c; padding: 15px; border-radius: 12px; margin-bottom: 25px; font-weight: 600; font-size: 0.9rem; border: 1px solid #fecaca;">
                <i class="fa-solid fa-circle-exclamation" style="margin-right: 8px;"></i> <?php echo $error; ?>
            </div>
        <?php endif; ?>

        <form action="edit-property.php?id=<?php echo $id; ?>" method="POST" enctype="multipart/form-data">
            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 30px;">
                <!-- Main Details -->
                <div style="display: grid; gap: 25px;">
                    <div class="form-group">
                        <label style="font-weight: 600; color: #475569; margin-bottom: 10px; display: block;">Listing
                            Title</label>
                        <input type="text" name="title" required
                            value="<?php echo htmlspecialchars($property['title']); ?>"
                            placeholder="e.g. Modern Villa in Gulberg"
                            style="width: 100%; padding: 12px 16px; border-radius: 12px; border: 1px solid #e2e8f0; font-family: inherit;">
                    </div>

                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                        <div class="form-group">
                            <label
                                style="font-weight: 600; color: #475569; margin-bottom: 10px; display: block;">Location</label>
                            <input type="text" name="location" required
                                value="<?php echo htmlspecialchars($property['location']); ?>"
                                placeholder="e.g. Bahria Town, Lahore"
                                style="width: 100%; padding: 12px 16px; border-radius: 12px; border: 1px solid #e2e8f0;">
                        </div>
                        <div class="form-group">
                            <label
                                style="font-weight: 600; color: #475569; margin-bottom: 10px; display: block;">Pricing
                                (Rs.)</label>
                            <input type="number" name="price" required value="<?php echo $property['price']; ?>"
                                style="width: 100%; padding: 12px 16px; border-radius: 12px; border: 1px solid #e2e8f0;">
                        </div>
                    </div>

                    <div class="form-group">
                        <label style="font-weight: 600; color: #475569; margin-bottom: 10px; display: block;">Property
                            Description</label>
                        <textarea name="description" rows="8" required
                            style="width: 100%; padding: 12px 16px; border-radius: 12px; border: 1px solid #e2e8f0; font-family: inherit; resize: vertical;"><?php echo htmlspecialchars($property['description']); ?></textarea>
                    </div>
                </div>

                <!-- Sidebar Details -->
                <div style="display: grid; gap: 25px; align-content: start;">
                    <div class="form-group">
                        <label style="font-weight: 600; color: #475569; margin-bottom: 10px; display: block;">Property
                            Image</label>
                        <div style="margin-bottom: 15px;">
                            <img src="../assets/images/<?php echo $property['image']; ?>"
                                style="width: 100%; border-radius: 16px; box-shadow: 0 4px 10px rgba(0,0,0,0.1);">
                            <p style="font-size: 0.75rem; color: #64748b; text-align: center; margin-top: 5px;">Current:
                                <?php echo $property['image']; ?>
                            </p>
                        </div>
                        <div
                            style="border: 2px dashed #cbd5e1; padding: 15px; border-radius: 16px; text-align: center; position: relative;">
                            <i class="fa-solid fa-camera-rotate"
                                style="font-size: 1.5rem; color: #94a3b8; margin-bottom: 8px; display: block;"></i>
                            <input type="file" name="image" accept=".png, .jpg, .jpeg"
                                style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; opacity: 0; cursor: pointer;">
                            <span style="font-size: 0.8rem; color: #64748b;">Update via PNG or JPG</span>
                        </div>
                    </div>

                    <div class="form-group">
                        <label
                            style="font-weight: 600; color: #475569; margin-bottom: 10px; display: block;">Availability
                            Status</label>
                        <select name="status" required
                            style="width: 100%; padding: 12px 16px; border-radius: 12px; border: 1px solid #e2e8f0; background: #fff; appearance: none; position: relative;">
                            <option value="Available" <?php echo $property['status'] == 'Available' ? 'selected' : ''; ?>>
                                Available</option>
                            <option value="Sold" <?php echo $property['status'] == 'Sold' ? 'selected' : ''; ?>>Sold
                            </option>
                            <option value="Rent" <?php echo $property['status'] == 'Rent' ? 'selected' : ''; ?>>For Rent
                            </option>
                        </select>
                    </div>

                    <div style="margin-top: 20px; display: flex; flex-direction: column; gap: 12px;">
                        <button type="submit" name="submit" class="btn"
                            style="width: 100%; margin-top: 0; border-radius: 12px; padding: 14px; font-weight: 600; background: #3498db;">
                            Update Listing
                        </button>
                        <a href="dashboard.php"
                            style="text-align: center; text-decoration: none; color: #64748b; font-weight: 600; font-size: 0.95rem; padding: 10px;">
                            Cancel Changes
                        </a>
                    </div>
                </div>
            </div>
        </form>
    </div>
</main>

<?php require_once 'includes/footer.php'; ?>